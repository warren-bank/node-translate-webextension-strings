...
const process_cli = async function(argv_vals) {
  ...
  for (let i=0; i < argv_vals["--output-language"].length; i++) {
    ...
    if (translated_strings_array.length !== input_strings_array.length) {
      if (argv_vals["--debug"]) {
        console.log('ERROR:', `Number of "${output_language_code}" translated strings is incorrect!`, `Expected #${input_strings_array.length} but received #${translated_strings_array.length} from server.`, {translated_strings_array})
        throw new Error('')
      }
      else {
...
const get_output_filepath = function(argv_vals, output_language_code) {
  ...
  if (argv_vals["--make-resource-dirs"]) {
    dname = output_language_code.replace('-', '_')
  }
  else {
