const translator = require('@warren-bank/multiapi-language-translator')
...
const process_cli = async function(argv_vals) {
  translator.set_api(argv_vals["--api-service"], argv_vals["--api-key"], argv_vals["--api-url"])
  ...
  const duplicates_store = new translator.DuplicatesStore(input_strings_array)
  ...
  const target_all_supported_output_languages = (!argv_vals["--output-language"] || !argv_vals["--output-language"].length)

  if (target_all_supported_output_languages) {
    await translator.init()

    argv_vals["--output-language"] = translator.get_output_languages(argv_vals["--input-language"])

    // if empty, throw error
    if (!argv_vals["--output-language"] || !argv_vals["--output-language"].length) {
      if (!translator.is_valid_input_language(argv_vals["--input-language"])) {
        throw new Error(
          'ERROR: Input language is not valid.' + "\n" +
          `You entered: "${argv_vals["--input-language"]}"` + "\n" +
          'Valid options: ' + JSON.stringify(translator.get_input_languages(), null, 2)
        )
      }
      else {
        throw new Error(
  ...
  for (let i=0; i < argv_vals["--output-language"].length; i++) {
    ...
    if (target_all_supported_output_languages) {
      // skip languages not supported by WebExtension
      try {
        get_webextension_resource_directory_name(output_language_code)
      }
      catch(e) {
        continue
      }
